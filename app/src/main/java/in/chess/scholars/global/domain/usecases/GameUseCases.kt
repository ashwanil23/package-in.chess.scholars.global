...
// CORRECTED: Added a new use case for updating the draw offer
/**
 * Use case to offer, accept, or decline a draw.
 */
class UpdateDrawOfferUseCase(private val gameRepository: GameRepository) {
    suspend operator fun invoke(gameId: String, offeringPlayerId: String?): DataResult<Unit> {
        return gameRepository.updateDrawOffer(gameId, offeringPlayerId)
    }
}
